 ...
 * Fixes recurring payment currency code issues with non-USD recurring payment profiles
 * patch for use with LifterLMS 2.7.12. Incompatible with LifterLMS 3.0 and greater.
 * 
 * https://lifterlms.com/docs/paypal-billing-agreements-2-x/
 * 
...
class LLMS_Payment_Gateway_Paypal extends LLMS_Payment_Gateway {
	...
	private $short_term = array(
		...
		'recurring_currency_code' => 'CURRENCYCODE',
	...
	public function complete_payment( $request, $order ) {
		...
		if ($order->payment_option == 'recurring' ) {
			...
			$param = array(
				...
				'billing_freq' => $order->billing_freq,
				'recurring_amount' => $order->product_price,
				'recurring_currency_code' => $request['CURRENCYCODE'],
				'token' => $request['TOKEN'],
				'payer_id' => $request['PAYERID'],
				'total_billing_cycles' => $order->billing_cycle,
				'max_failed_payments' => '1',
